    ...
    if (AppObject.length !== 0) {
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                } else if (PressedApplication == "dark-chat") {
                    if (QB.Phone.Data.IsOpen) {
                        $.post('https://qb-phone/GetChat', JSON.stringify({}), function(Chat){
                            QB.Phone.Notifications.LoadChatMessage(Chat.ChatData, Chat.hasVPN);
                        });
                    }
...
QB.Phone.Functions.Open = function(data) {
    QB.Phone.Animations.BottomSlideUp('.container', 500, -6.6);
    QB.Phone.Notifications.LoadChatMessage(data.Chats, data.hasVPN);
    QB.Phone.Data.IsOpen = true;
}
    ...
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            ...
            case "UpdateTweets":
                ...
                break;
            case "UpdateChat":
                if (QB.Phone.Data.currentApplication == "dark-chat") {
                    QB.Phone.Notifications.LoadChatMessage(event.data.Chats, event.data.hasVPN);
                }
                break;
            case "RefreshDarkWeb":
